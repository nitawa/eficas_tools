...
from InterfaceGUI.QT6.monPlusieursBasePanel import MonPlusieursBasePanel
...
class MonFonctionPanel(MonPlusieursBasePanel):
    ...
    def decoupeListeValeurs(self, liste):
        ...
        if len(liste) % self.nbValeursASaisir != 0 and (len(liste) % self.nbValeurs):
            ...
            self.editor.afficheMessageQt(message, Qt.GlobalColor.red)
        i = 0
        while i < len(liste):
            try:
    ...
    def ajout1Valeur(self, liste=[]):
        ...
        if not validite:
            self.editor.afficheMessageQt(comm, Qt.GlobalColor.red)
        else:
