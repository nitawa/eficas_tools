            ...
            for regle in listeRegles:
                ...
                for ligne in texteRegle.split("\n"):
                    ...
                    if ligne[0] == "\t":
                        ligne = "     " + ligne[1:]
                    if test:
                        self.liste.append((ligne, Qt.GlobalColor.black))
                    else:
                        self.liste.append((ligne, Qt.GlobalColor.red))
                self.liste.append(("", Qt.GlobalColor.red))
        if self.liste == []:
            self.liste.append(
                ("pas de regle de construction pour ce jeu de commandes", Qt.GlobalColor.black)
            )
